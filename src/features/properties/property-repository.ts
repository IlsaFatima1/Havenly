...
function handleDbError(error: any): never {
  ...
  if (msg.toLowerCase().includes('relation') && (msg.toLowerCase().includes('exist') || msg.toLowerCase().includes('find'))) {
    throw new Error(`Database setup is incomplete: 'properties' table does not exist in your Supabase project yet. Please copy the SQL queries inside 'SETUP_SUPABASE_TABLE.sql' (located in the project root directory) and execute them in your Supabase Dashboard SQL Editor to initialize your databases and security policies correctly.`);
  }
...
export const propertyRepository={
 ...
 async create(input:PropertyInput,status:PropertyStatus){
   if(supabase){
     ...
     if(!user) throw new Error('Sign in again before adding a property.');
